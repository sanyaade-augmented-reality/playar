...
<?php if(isset($this->results)) { ?>
		<div id="mapview_listview" class="results">
			<table class="results" cellpadding="4" cellspacing="0">
				<tr>
					<td style="vertical-align:middle; white-space: nowrap; overflow: hidden; padding: 0px; background-color: #000000;"  width="50%">
						<span id="location-area" class="small" style=" color: #ffffff; font-size:12px; ">
						<form id="location-input" onsubmit="geocodeAddress('<?php echo $this->geo_bias ?>'); return false;" >
							...
							<input type="submit" value="Ma position" class="stndsmall" style="font-size: 11px">
						</form>
						<div id="did-you-mean" style="display:none;">
						</div>
						</span>
					</td>
					<td  valign="top" style="white-space: nowrap; overflow: hidden; background-color: #000000">
						<p align="right"><span class="small" style="color: #ffffff;  background-color: #000000">
						<?php if($this->view_type == MAP_VIEW) { ?>
							<a href="<?php echo $this->oar_browser_path ?>?url=<?php echo $this->url ?>&<?php echo $this->get_current_url('viewType', LIST_VIEW, 'pageKey',0, true, true); ?>">Liste</a>
						<?php } else { ?>
							<b style="font-size: 90%">Liste</b>
						<?php } ?>	
							&nbsp;|&nbsp; 
						<?php if($this->view_type == LIST_VIEW) { ?>
							<a href="<?php echo $this->oar_browser_path ?>?url=<?php echo $this->url ?>&<?php echo $this->get_current_url('viewType', MAP_VIEW, 'pageKey',0,true, true); ?>">Carte</a>
						<?php } else { ?>
							<b style="font-size: 90%">Carte</b>
						<?php } ?>	 
						</span></p>
					</td>
				</tr>
...
<?php } ?>
    <table class="results" cellpadding="4" cellspacing="0" style="color:white;">
<?php 
	$cnt = 0;
	...
	?>
		...
		<tr>
			<td class="resultstext" <?php if($cnt%2 == 1) { ?>bgcolor="#0f0f0f"<?php } else { ?>bgcolor="#353535"<?php } ?>>
				...
				<?php if($hotspot['actions'][0]['layerURL'] == 1) { ?>
				 &nbsp;&nbsp;&nbsp;<span nowrap class="small" style="text-align: right; font-weight:bold; color: #ffffff;"><a href="<?php echo $oar_browser_path . '?url=' . $hotspot['actions'][0]['uri'] . '&lat=' . $this->latitude . '&lon=' . $this->longitude ?>">Layer</a> &raquo;</span>
		<?php } ?>
			<br/>
			...
			<?php if(isset($hotspot['actions'][1]['uri'])) { ?>
				...
				<div  id="more<?php echo $cnt ?>" style="display: none; background-color:#373737; padding: 5px">
					<span class="regular">
			...
			<td class="distance" <?php if($cnt%2 == 0) { ?>bgcolor="#0f0f0f"<?php } else { ?>bgcolor="#353535"<?php } ?> align="center"><span class="regular"><?php if($hotspot['distance'] > 1000) {
...
saddr=42.35892,-71.05781&daddr=40.756054,-73.986951">Test link</a>-->
				 <a href="http://maps.google.com/maps?f=d&hl=en&doflg=ptk&q=<?php echo ($hotspot['lat']/1000000.0) ?>%2C<?php echo ($hotspot['lon']/1000000.0) ?>&ie=UTF8&om=1">Direction</a> <?php /* Have changed it around so that you see the destination as your starting point &ll=<?php echo $this->latitude ?>%2C<?php echo $this->longitude ?> maps.google.co.uk     &daddr=<?php echo ($hotspot['lat']/1000000.0) ?>%2C<?php echo ($hotspot['lon']/1000000.0) ?>&saddr=<?php echo $this->latitude ?>%2C<?php echo $this->longitude ?>  */ ?></span>
			</td>

			<td class="pics" valign="center" style=" background-color: #000000">
...
<?php if($this->rvolve_user_id != 0) { 
	...
	<p class="small"><span id="rvolve-ads"></span></p>
