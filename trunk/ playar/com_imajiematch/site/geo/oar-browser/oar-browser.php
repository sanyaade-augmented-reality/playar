...
if(isset($_REQUEST['url'])) {
	if($query_string = $oar->get_layer_query_str()) {
		...
		$oar->error_message = "D&eacute;sol&eacute; ! Il n'y a pas d'ImajieMatch dans ton secteur.<br/>";
	}
					...
					if(count($results) > 0) {
						 if(is_array($results)) {
						     if($results['errorCode'] == 0) {
						  	...
						  	echo $results['errorString'];
						     }
						 
						  } else {
						  	?>
						  	Invalide.  Les messages d'erreurs suivant ont &eacute;t&eacute; enregist&eacute;s:
						  	<?php	
						  	echo $results;
						  }
