...
class clsCSVImport
{
	...
	private function set_peanos($id_array)
	{
		...
		$sql = "SELECT int_point_id, dec_latitude, dec_longitude FROM saj_imajiematch_layar_poi WHERE int_peano1 = 0";
		...
		while ($row = mysql_fetch_assoc($result)) {
			...
			$sql = "UPDATE saj_imajiematch_layar_poi SET int_peano1 = '" . $peano1 ."',
							int_peano2 = '" . $peano2 . "',
		...
		foreach ($lines as $line_num => $line) {
    			...
    			if($first_line == true) {
    				...
    				if($line != "") {
	    				...
	    				$sql = "INSERT INTO saj_imajiematch_layar_poi (" . $this->fields . ") VALUES (" . $line . ")";
	...
	public function delete_all()
	{
		...
		$sql = "DELETE FROM saj_imajiematch_layar_poi";
		$result = mysql_query($sql) or die("Unable to execute query $sql " . mysql_error());

		$sql = "ALTER TABLE saj_imajiematch_layar_poi AUTO_INCREMENT=1";
		$result = mysql_query($sql) or die("Unable to execute query $sql " . mysql_error());
	}
	...
	{
		//Delete a single row
		$sql = "DELETE FROM saj_imajiematch_layar_poi WHERE int_point_id = " . $point_id;
		$result = mysql_query($sql) or die("Unable to execute query $sql " . mysql_error());
