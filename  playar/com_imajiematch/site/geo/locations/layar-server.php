<?php 
	...
	$ly_params = array('layar_name' => layer_name($db_name),
				...
				'layerURL' => "layerURL",
				'type' => "type",
				'debug' => true);		//Switch to false when taking server live  TEMP to TRUE
	...
	$params = array('latitude' => $ly->layar_latitude,		//Latitude in decimal degrees of center of search
		...
		'table_name' => "saj_imajiematch_layar_poi",		//Main table name that is being searched on 
	...
	if($merge_with != "") {
		...
		$params['max_records'] = 2;				//We only want a couple of results from the secondary
									//layer
		$ly->layar_attribution = "imajie.tv";		//Insert some branding
