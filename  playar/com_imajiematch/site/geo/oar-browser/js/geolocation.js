...
function map_finish_mesage() {
	...
	if (atl_scenario == 'manage') {
	    ...
	    finish_message = "Tu est pr&ecirc;t &agrave; continuer.";
	} else {
	    if(atl_scenario == 'adminpanel') {
	    	finish_message = "Copie la latitude et la longitude dans les champs ci-haut.";
	    } else {
	    	finish_message = "Pr&ecirc;t pour le Match !"; //"You are ready to find products from this location";
	    	//document.lpf.s.focus();
	    }
	}
    } else {
	finish_message = "Cherchez &agrave; partir d'ici...";
    }
...
function location_found_message(accuracy, via_map_or_geo) {
    ...
    var fine_tune_vs_edit = (accuracy >= 1000) ? 'Ajuste&nbsp;ta&nbsp;Position' : 'Change&nbsp;ta&nbsp;Position';
    if (accuracy > 1000) {
	// round the accuracy to the nearest km
	readable_accuracy = parseInt((accuracy + 500) / 1000) + ' km';
    } else {
	readable_accuracy = parseInt(accuracy) + ' m&ecirc;tres';
    }
    var location_verb = (via_map_or_geo == 'geo') ? 'Geolocated' : 'located';
    var location_sentence = 'Pr&eacute;cision: ' + readable_accuracy + '.';
    ...
    if(atl_scenario == 'adminpanel') {
    	...
    	var saveLocationButton = '<button onclick="set_location_cookie((document.lpf.lat.value), (document.lpf.lon.value), (document.lpf.acc.value)); return false;" style="margin:3px; font-weight:normal"><small>Valider</small></button>';
    }
...
function map_message() {
    document.getElementById("location_message").innerHTML = "<span class=\'phone-only\' >Double </span>Clique pour zoomer et placer ta position<br/>" + clear_location_link() + ' ' + close_map_link();
    return;
}
...
function close_map_link() {
    return "<button onclick=\"close_map(); return false;\" style=\"margin:3px; font-weight:normal\"><small>Terminer</small></button>";
}

function clear_location_link() {
    return "<button onclick=\"document.lpf.lat.value=0;document.lpf.lon.value=0;document.lpf.acc.value=0;initialize_oar_map(48.41826449418743,-71.23947143554687,400000); clear_location_cookie(); return false;\" style=\"margin:3px; font-weight:normal\"><small>Reset</small></button>";
}
